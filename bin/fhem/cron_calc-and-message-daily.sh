...
# 16.02.2022 Variablen für YES und DBY umbenannt
...
# Das Datum von gestern (YES) und vorgestern (DBY) berechnen
YESTERDAY=`date -d "1 day ago" '+%d.%m.%Y'`
YES_ISO=`date -d "1 day ago" '+%Y-%m-%d'`
...
DELETE=$($PSQL -X $DBNAME -t -c "delete from history where date_trunc('day', timestamp)= '$YES_ISO' and device = 'MQTT2_DVES_0371A9' and reading = 'ENERGY_Yesterday' and timestamp !=(select min(timestamp) from history where date_trunc('day', timestamp)= '$YES_ISO' and device = 'MQTT2_DVES_0371A9' and reading = 'ENERGY_Yesterday');")
...
TOTAL1=`$PSQL -X $DBNAME -t -c "SELECT DISTINCT ON (timestamp::date) value FROM history where reading='total_consumption' AND timestamp::date='$YES_ISO' ORDER BY timestamp::date DESC, timestamp DESC;"`
...
$PSQL -X $DBNAME -t -c "insert into current (timestamp, device, type, event, reading, value, unit) values ('$(date -d "$YES_ISO" "+%Y-%m-%d 23:59:59")', 'Stromzaehler', 'OBIS', 'daily_consumption: $CONSUMPTION', 'daily_consumption', $CONSUMPTION, '');"
$PSQL -X $DBNAME -t -c "delete from history where reading='daily_consumption' and timestamp >= '$YES_ISO';"
$PSQL -X $DBNAME -t -c "insert into history (timestamp, device, type, event, reading, value, unit) values ('$(date -d "$YES_ISO" "+%Y-%m-%d 23:59:59")', 'Stromzaehler', 'OBIS', 'daily_consumption: $CONSUMPTION', 'daily_consumption', $CONSUMPTION, '');"

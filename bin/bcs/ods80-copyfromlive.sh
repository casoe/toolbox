...
FULLDATE=`date +\%Y-\%m-\%d`
BACKUP=/opt/projektron/bcs/backup/ods80_backup_${FULLDATE}.zip

echo Dump von Live-ODS
$PGDUMP -v -Fc -h 172.16.1.103 -U bcs --no-password -f $BACKUP ods80

echo Drop and create database
su -c "dropdb --if-exists ods80" postgres
su -c "createdb ods80" postgres

echo Restore auf Test-ODS
$PGRESTORE -v -c -w -n public -h localhost -p 5432 -d ods80 -U bcs $BACKUP

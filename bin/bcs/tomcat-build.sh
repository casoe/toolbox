...
elif [ -d "/usr/lib/jvm/adoptopenjdk-8-hotspot-amd64" ]; then
  MY_JAVA_HOME="/usr/lib/jvm/adoptopenjdk-8-hotspot-amd64"
elif [ -d "/usr/lib/jvm/temurin-8-jdk-amd64" ]; then
  MY_JAVA_HOME="/usr/lib/jvm/temurin-8-jdk-amd64"
else
  echo "No JDK found"
  exit 1
fi
